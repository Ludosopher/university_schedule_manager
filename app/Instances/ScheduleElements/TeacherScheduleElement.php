            ...
            if (!in_array($g_lesson->teacher->id, $looked_teachers)) {
                ...
                if ($is_suitable_teacher) {
                    foreach ($g_lesson->teacher->lessons as $dt_lesson) {
                        ...
                        if (isset($week_number) && isset($dt_lesson->study_period_id) && ! DateHelpers::checkRegularLessonToWeek($dt_lesson->study_period_id, $week_number)) {
                            continue;
                        }
                        if (! DateHelpers::checkOneTimeLessonToWeek($week_number, $dt_lesson->date)) {
                            continue;
                        };
                        ...
                        if (count($dt_lesson_groups_ids) == count($groups_ids) && $dt_lesson_groups_ids === $groups_ids) {
                            foreach ($seeking_teacher->lessons as $st_lesson) {
                                ...
                                if (isset($week_number) && isset($st_lesson->study_period_id) && ! DateHelpers::checkRegularLessonToWeek($st_lesson->study_period_id, $week_number)) {
                                    continue;
                                }
                                if (! DateHelpers::checkOneTimeLessonToWeek($week_number, $st_lesson->date)) {
                                    continue;
                                };
