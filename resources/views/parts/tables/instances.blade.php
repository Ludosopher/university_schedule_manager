...
<h1>{{ __('header.'.$data['appelation_plural_name']) }}</h1>
    ...
    <tbody>
        @foreach($data['instances'] as $instance)
            <tr>
                @if (Auth::check() && (Auth::user()->is_admin || Auth::user()->is_moderator))
                    <td>
                        <a class="" href="{{ route($data['appelation'].'-update-form', ['updating_id' => $instance->id]) }}" title="{{ __('title.update') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                            </svg>
                        </a>
                        <a class="" href="{{ route($data['appelation'].'-delete', ['deleting_id' => $instance->id]) }}" title="{{ __('title.delete') }}">
                ...
                @foreach($data['table_properties'] as $property)
                    ...
                    @if(is_array($field) && count($field) > 1)
                        @php
                            $value = $instance;
                            if ($data['appelation'] === 'lesson') { $is_week_day = false; }
                            foreach ($field as $part) {
                                if ($data['appelation'] === 'lesson') { 
                                    if ($part === 'week_day') {
                                        $is_week_day = true;
                                    } 
                                }
                        ...
                        @if($data['appelation'] === 'lesson')
                            @if($is_week_day && isset($instance->date))
                                <td class="regular-cell">{{ \Lang::has('dictionary.'.$value) ? __('dictionary.'.$value) : $value }} ({{ $instance->date }})</td>
                            @else
                        ...
                        @else
                            <td class="regular-cell">{{ \Lang::has('dictionary.'.$value) ? __('dictionary.'.$value) : $value }}</td>    
                        @endif
                    @elseif(($data['appelation'] === 'teacher' && $field == 'full_name') || ($data['appelation'] === 'group' && $field == 'name'))
                        <td class="regular-cell"><a href="{{ route($data['appelation'].'-schedule', ['schedule_'.$data['appelation'].'_id' => $instance->id]) }}" title="{{ __('title.'.$data['appelation'].'_schedule') }}">{{ \Lang::has('dictionary.'.$instance->$field) ? __('dictionary.'.$instance->$field) : $instance->$field }}</a></td>
                    @else
